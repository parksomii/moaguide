...
public class LocalBillingService {
	...
	public void save(String nickname, String cardCompany, Integer cardNumber, String customerKey,
		String billingKey) throws DuplicateKeyException, SQLIntegrityConstraintViolationException {
	...
	public void start(String nickname, String secretkey, String orderId) throws Exception {
		...
		for (int i = 0; i < 12; i++) {
			LocalDateTime endDate = LocalDateTime.now().plusDays(1 + i).plusHours(0).withMinute(30)
		...
		cardRepository.updateSubscript(nickname, requestedAt,
	...
	public void startWithCoupon(String nickname, Long couponId, String orderId) throws Exception {
		...
		paymentLogRepository.save(
			new PaymentLog("모아가이드 " + couponmonth + "개월 무료구독", orderId, 0, "쿠폰", now_date, now_date,
				4900, nickname));
		couponUserRepository.updateRedeemedWithCouponId(true, LocalDate.now(), nickname, couponId);
		userRepository.updateRole(nickname, Role.VIP);
		cardRepository.updateSubscript(nickname, now_date,
			paymentRequests.get(0).getNextPaymentDate());
	}
	...
	public LocalSubscriptDateDto findDate(String nickname) {
		return cardRepository.findDate(nickname)
			.orElse(new LocalSubscriptDateDto(null, null, null));
	}
	...
	public void startWithDate(String nickname, LocalDateTime nextPaymentDay) {
		List<LocalPaymentRequest> paymentRequests = new ArrayList<>();
		for (int i = 0; i < 12; i++) {
			LocalDateTime endDate = nextPaymentDay.plusDays(i).plusHours(0).withMinute(30)
				.withSecond(0).withNano(0);
			String uniqueKey = UUID.randomUUID().toString(); // 첫 번째 UUID는 이미 추가
			paymentRequests.add(new LocalPaymentRequest(uniqueKey, nickname, 4900, endDate, 0));
		}
