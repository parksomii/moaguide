...
public interface StayRateRepository extends JpaRepository<StayRate, Integer> {
	...
	@Query("SELECT new com.moaguide.refactor.building.dto.StayRateDto(s.day,s.rate,s.value) FROM StayRate s where s.keyword =:keyword and YEAR(s.day) between :startyear and :endyear order by s.day")
