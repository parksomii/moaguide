...
public class CouponService {
	...
	public void firstCreate(String email) {
		try {
			...
			couponAdminRepository.save(
				new CouponAdmin(null, couponname, couponNumber, today, 1, nickname));
			CouponAdmin Coupon = couponAdminRepository.findByCodeAndNickname(couponNumber, nickname)
				.orElseThrow();
	...
	public String create(int month, String nickname) {
		try {
			...
			couponAdminRepository.save(
				new CouponAdmin(null, couponname, couponNumber.toString(), today, month, nickname));
			return result;
		} catch (Exception e) {
			return "쿠폰생성시 오류 발생";
		}
	...
	public int valid(String code, String nickname) {
		CouponAdmin coupon = couponAdminRepository.findByCodeAndNickname(code, nickname)
			.orElse(null);
		if (coupon == null) {
			return -1;
		} else {
			try {
				couponUserRepository.save(
					new CouponUser(null, nickname, coupon.getId(), false, null));
				return 1;
