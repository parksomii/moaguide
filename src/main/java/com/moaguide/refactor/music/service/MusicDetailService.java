...
import com.moaguide.refactor.music.dto.MusicDivideDto;
...
import com.moaguide.refactor.music.dto.SteamingDto;
...
public class MusicDetailService {
	...
	public MusicPublishDto findBase(String productId) {
		...
		MusicPublishDto publishDto = new MusicPublishDto(
			...
			result[6] == null || "1990-01-01".equals(result[6].toString()) ? ""
				: ((Date) result[6]).toLocalDate().toString()
		);
	...
	public MaxAndMinDto findStreaming(String productId, int month) {
		...
		for (Object[] result : resultList) {
			...
			if (steamingDto.getValue() > max) {
				max = steamingDto.getValue();
			}
			if (steamingDto.getValue() < min) {
				min = steamingDto.getValue();
			}
